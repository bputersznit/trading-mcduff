...
namespace NinjaTrader.NinjaScript.Strategies
{
    public class CG_L2_Capture_Chunked : Strategy
    {
        ...
        protected override void OnStateChange()
        {
            ...
            {
                // Close any open chunk when strategy is disabled or terminated
                CloseCurrentChunk();

                if (totalRowsCaptured > 0 && State == State.Terminated)
                {
        ...
        protected override void OnMarketDepth(MarketDepthEventArgs e)
        {
            ...
            // Determine side by comparing to mid price
            double mid = (GetCurrentBid() + GetCurrentAsk()) / 2.0;
            char side = (e.Price < mid) ? 'B' : 'A';
            ...
            // Flush every 1000 rows for safety
            if (rowsInCurrentChunk % 1000 == 0)
            {
                currentChunk.Flush();
            }
        ...
        private void CloseCurrentChunk()
        {
            if (currentChunk != null)
            {
                ...
                {
                    Print($"Error closing chunk {chunkIndex}: {ex.Message}");
                }
                finally
                {
                    currentChunk = null;
