//
// CG MNQ Scalping Strategy - NT8 NATIVE VERSION v3 - TREND-AWARE
// Major improvements for asymmetric market conditions:
...
{
	public class CGScalpingStrategyNT8Native_v3 : Strategy
	{
		#region Variables
		...
		protected override void OnStateChange()
		{
			...
			{
				Description = @"NT8 Native Order Flow Scalping - v3 TREND-AWARE";
				Name = "CGScalpingStrategyNT8Native_v3";
			...
			else if (State == State.DataLoaded)
			{
				...
				Print("========================================");
				Print("CG SCALPING NT8 NATIVE v3 - TREND-AWARE");
				Print("========================================");
				Print("NEW v3 FEATURES:");
		...
		// NEW v3: Helper to check if price is near recent swing low
		...
		// NEW v3: Helper to check if price is near recent swing high
		...
		// NEW v3: Get recent high/low for breakout detection
		private void GetRecentHighLow(int bars, out double high, out double low)
		{
			high = High[0];
			low = Low[0];

			for (int i = 1; i < Math.Min(bars, CurrentBar); i++)
			{
		...
		{
			if (orderFlowHistory.Count < 5 || CurrentBar < 20)
				return null;
			...
			// NEW v3: Get trend direction and strength
			bool upTrend = emaFast[0] > emaSlow[0];
			bool downTrend = emaFast[0] < emaSlow[0];
			...
			if (recentSellVolume > AbsorptionMinAggressor)
			{
				...
				if (nearBidSize > recentSellVolume * AbsorptionRatio)
				{
					...
					double priceChange = Close[0] - Close[3];
		...
		private Signal DetectBreakout()
		{
			...
			// NEW v3: Get trend and recent price levels
			bool upTrend = emaFast[0] > emaSlow[0];
			bool downTrend = emaFast[0] < emaSlow[0];
