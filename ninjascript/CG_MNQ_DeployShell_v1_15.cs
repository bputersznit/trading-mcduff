// CG_MNQ_DeployShell_v1_15.cs
// NinjaTrader 8 Strategy
// Generated: 2026-05-09 11:45:00 America/New_York
//
// PURPOSE
//   Rolling BOS/CHOCH + premarket-context forward-test shell for MNQ.
//
// WHY v1.15 EXISTS
...
//   rolling structure breaks. v1.15 changes the signal brain:
//
...
{
    public class CG_MNQ_DeployShell_v1_15 : Strategy
    {
        ...
        [NinjaScriptProperty]
        [Display(Name = "RequireRetestForEntry", GroupName = "Structure", Order = 18)]
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                TelemetryCsvPath = Path.Combine(defaultDir, "CG_MNQ_DeployShell_v1_15_telemetry.csv");
            }
            else if (State == State.Configure)
            {
                ...
                SetProfitTarget("CGV115_LONG", CalculationMode.Ticks, TargetTicks);
                SetStopLoss("CGV115_LONG", CalculationMode.Ticks, StopTicks, false);
                SetProfitTarget("CGV115_SHORT", CalculationMode.Ticks, TargetTicks);
                SetStopLoss("CGV115_SHORT", CalculationMode.Ticks, StopTicks, false);
            }
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (Position.Quantity > HardQuantity)
            {
                ...
                ExitLong("CGV115_EmergencyQty_Long", "CGV115_LONG");
                ExitShort("CGV115_EmergencyQty_Short", "CGV115_SHORT");
                return;
            }
            ...
            if (EnableDailyLossLock && !dailyLocked)
            {
                ...
                if (realizedToday <= -Math.Abs(DailyLossLockUsd))
                {
                    ...
                    if (Position.MarketPosition == MarketPosition.Long)
                        ExitLong("CGV115_DailyLock_Long", "CGV115_LONG");
                    else if (Position.MarketPosition == MarketPosition.Short)
                        ExitShort("CGV115_DailyLock_Short", "CGV115_SHORT");
                    return;
                }
            }
            ...
            {
                if (Position.MarketPosition == MarketPosition.Long)
                    ExitLong("CGV115_RuntimeSpread_Long", "CGV115_LONG");
                else if (Position.MarketPosition == MarketPosition.Short)
                    ExitShort("CGV115_RuntimeSpread_Short", "CGV115_SHORT");
            ...
            if (Position.MarketPosition != MarketPosition.Flat && activeEntryTime != Core.Globals.MinDate)
            {
                ...
                {
                    if (Position.MarketPosition == MarketPosition.Long)
                        ExitLong("CGV115_TimeExit_Long", "CGV115_LONG");
                    else if (Position.MarketPosition == MarketPosition.Short)
                        ExitShort("CGV115_TimeExit_Short", "CGV115_SHORT");
        ...
        private void UpdateRollingStructure(DateTime now)
        {
            ...
            if (longBos)
            {
                ...
                LogTelemetry(isChoch ? "CHOCH_LONG" : "BOS_LONG", now, GetSessionPhase(now),
                    string.Format(CultureInfo.InvariantCulture, "recentHigh={0};price={1};mom={2}", recentHigh, current, momentum));
            }
            else if (shortBos)
            {
                bool isChoch = structureBias > 0;
                ...
                LogTelemetry(isChoch ? "CHOCH_SHORT" : "BOS_SHORT", now, GetSessionPhase(now),
                    string.Format(CultureInfo.InvariantCulture, "recentLow={0};price={1};mom={2}", recentLow, current, momentum));
            }

            UpdateRetestArms(current);
        }

        private void UpdateRetestArms(double current)
        {
            if (structureBias == 1)
            {
        ...
        private bool EvaluateLongEntry(DateTime now)
        {
            ...
            if (contextOk && (impulseEntry || retestEntry))
            {
                longImpulseArmed = false;
                longRetestArmed = false;
                return true;
        ...
        private void SubmitEntry(DateTime now, string phase, string side)
        {
            ...
            {
                activeEntryTime = now;
                EnterLong(HardQuantity, "CGV115_LONG");
                LogTelemetry("ENTRY_SUBMITTED_LONG", now, phase, "rolling_bos_choch_context");
            }
            else
            {
                activeEntryTime = now;
                EnterShort(HardQuantity, "CGV115_SHORT");
        ...
        private void MaybePrintStatus(DateTime now, string phase, double spreadTicks)
        {
            ...
            string msg = string.Format(CultureInfo.InvariantCulture,
                "CGV115 STATUS {0} phase={1} pos={2} trades={3} spread={4:F1} mom={5:F1} bias={6}/{7} pmSeen={8} pmH={9:F2} pmL={10:F2} pmMid={11:F2} armedL={12}/{13} armedS={14}/{15} Lconf={16} Sconf={17} blocks[rth={18},spr={19},flat={20},cool={21},sess={22},struct={23},sig={24}] entries={25}",
            ...
            string nativeError)
        {
            ...
            if (order.Name == "CGV115_LONG" || order.Name == "CGV115_SHORT")
            {
