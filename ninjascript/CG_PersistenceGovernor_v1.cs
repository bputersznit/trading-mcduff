...
// Generated: 2026-05-07 09:35:00 EDT
//
// MNQ CG PersistenceGovernor v1 — GOVERNED EXECUTION BUILD
//
// PURPOSE
// -------
    ...
    public class CG_PersistenceGovernor_v1 : Strategy
    {
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                PersistenceDecay = 0.82;
                TrendOwnershipThreshold = 18.0;
                MinEntryPersistenceScore = 18.0;
                AllowRotationEntries = false;
                RequireConfirmedStateForEntry = true;
                RequireTwoBarsSameDirection = true;
                PostStopCooldownSeconds = 75;
                AntiFlipCooldownSeconds = 120;
                FlipOverrideExpansion = 2.50;
        ...
        private bool PassesPersistenceGovernor(bool longCandidate, bool shortCandidate, DateTime decisionTime)
        {
            ...
            if (RequireTwoBarsSameDirection && sameDirectionSyntheticBars < 2)
            {
                persistenceBlocked++;
                PrintBlock(decisionTime, "BLOCK_ONE_BAR_IMPULSE");
                return false;
            }
        ...
        private void PrintDiagnostic(DateTime time, string label)
        {
            ...
            Print(string.Format("{0:yyyy-MM-dd HH:mm:ss.fff} {1} syn={2} state={3} regime={4} persist={5:F2} sameDir={6} energy={7:F2} exp={8:F2} range={9:F1} p50={10:F1} p80={11:F1} pos={12} L={13} S={14} pctBlk={15} expBlk={16} energyBlk={17} stateBlk={18} persistBlk={19} rotBlk={20} flipBlk={21} postStopBlk={22} coolBlk={23} flatBlk={24} rthBlk={25} warmBlk={26} states={27}",
                time,
                label,
                syntheticBarId,
                auctionState,
        ...
        private void PrintAction(DateTime time, string action, string signal)
        {
            ...
            Print(string.Format("{0:yyyy-MM-dd HH:mm:ss.fff} {1} signal={2} syn={3} state={4} regime={5} persist={6:F2} sameDir={7} energy={8:F2} exp={9:F2} range={10:F1} p50={11:F1} p80={12:F1} pos={13}",
                time, action, signal, syntheticBarId, auctionState, trendRegime, directionalPersistenceScore, sameDirectionSyntheticBars, auctionEnergy, lastExpansionScore, lastSyntheticRange, p50Range, p80Range, Position.MarketPosition));
        }
