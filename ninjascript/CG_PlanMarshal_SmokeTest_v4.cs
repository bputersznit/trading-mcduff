...
// Default trigger is tick-based: EntryEveryNTicks = 1.
// v4 fix: if protective stop would be invalid, flatten immediately instead of submitting invalid stop.
// Fixes:
// - Tick-count based trigger instead of minute-based trigger.
...
{
    public class CG_PlanMarshal_SmokeTest_v4 : Strategy
    {
        ...
        private long ticksSinceLastEntry = 0;
        ...
        [NinjaScriptProperty]
        [Display(Name = "EntryEveryNTicks", Order = 7, GroupName = "Signal")]
        public int EntryEveryNTicks { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "ExpectedAccountName", Order = 8, GroupName = "Diagnostics")]
        public string ExpectedAccountName { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "AlternateLongShort", Order = 9, GroupName = "Signal")]
        public bool AlternateLongShort { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "ForceLongOnly", Order = 10, GroupName = "Signal")]
        ...
        [NinjaScriptProperty]
        [Display(Name = "SafetyTicksAwayFromMarket", Order = 14, GroupName = "Orders")]
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name = "CG_PlanMarshal_SmokeTest_v4";
        ...
        private TestDirection GetSmokeTestSignal()
        {
            ...
            // Tick-based trigger:
            // With Calculate.OnEachTick, this increments once per OnBarUpdate tick.
            // EntryEveryNTicks = 1 means the first eligible tick after flat/time gates may signal.
            ticksSinceLastEntry++;

            if (ticksSinceLastEntry < EntryEveryNTicks)
                return TestDirection.None;
            ...
            long bucket = ticksSinceLastEntry / Math.Max(1, EntryEveryNTicks);
        ...
        private void SubmitProtectiveBracketFromFill(string entryName, MarketPosition pos, double fillPrice, int qty)
        {
            ...
            if (pos == MarketPosition.Long)
            {
                ...
                // A sell stop for a long must be below current market.
                // If playback has already traded through the intended stop by the time the fill callback arrives,
                // submitting the stop is invalid. The safest smoke-test behavior is immediate market flatten.
                if (stopPrice >= bid)
                {
                    DPrint("EMERGENCY_FLAT",
                        "LONG intended stop invalid. fill=" + fillPrice.ToString("F2") +
                        " intendedStop=" + stopPrice.ToString("F2") +
                        " bid=" + bid.ToString("F2") +
                        " ask=" + ask.ToString("F2") +
                        " -> ExitLong market now.");
                    ExitLong(qty, "PM4_Long_EmergencyFlat", entryName);
                    bracketSubmitted = false;
                    return;
                }
                ...
                ExitLongStopMarket(0, true, qty, stopPrice, "PM4_Long_Stop", entryName);
                ExitLongLimit(0, true, qty, targetPrice, "PM4_Long_Target", entryName);
            }
            else if (pos == MarketPosition.Short)
            {
                ...
                // A buy stop for a short must be above current market.
                // If playback has already traded through the intended stop by the time the fill callback arrives,
                // submitting the stop is invalid. The safest smoke-test behavior is immediate market flatten.
                if (stopPrice <= ask)
                {
                    DPrint("EMERGENCY_FLAT",
                        "SHORT intended stop invalid. fill=" + fillPrice.ToString("F2") +
                        " intendedStop=" + stopPrice.ToString("F2") +
                        " bid=" + bid.ToString("F2") +
                        " ask=" + ask.ToString("F2") +
                        " -> ExitShort market now.");
                    ExitShort(qty, "PM4_Short_EmergencyFlat", entryName);
                    bracketSubmitted = false;
                    return;
                }
                ...
                ExitShortStopMarket(0, true, qty, stopPrice, "PM4_Short_Stop", entryName);
                ExitShortLimit(0, true, qty, targetPrice, "PM4_Short_Target", entryName);
            }
            ...
            MarketPosition marketPosition, string orderId, DateTime time)
        {
            ...
            if (execution.Order.Name.Contains("_Stop") ||
                ...
                execution.Order.Name.Contains("EmergencyFlat") ||
        ...
        private void DPrint(string tag, string message)
        {
            ...
            Print("[CG_PM_SMOKE_V4][" + tag + "] " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") + " | " + message);
        }
