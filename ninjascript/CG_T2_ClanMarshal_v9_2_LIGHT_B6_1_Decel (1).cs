...
// Generated: 2026-05-06 15:36:00 America/New_York
//
// MNQ Intraday Strategy - v9.2 LIGHT B6_1 Decel
//
// PURPOSE
//   Same file/class name as requested:
//     CG_T2_ClanMarshal_v9_2_LIGHT_B6_1_Decel
//
//   This is a sanitized replacement for the prior B6_1 build.
//   It keeps the loosened deceleration logic, but avoids possible NT8 UI/render
//   troublemakers:
//     - no decimal in class/name
//     - no special Unicode markers
//     - no BarsPeriod inspection
//     - no rendering/drawing code
//     - no AddDataSeries
//
...
// CORE LOGIC
//   LONG:
//     Down impulse -> reclaim -> low is not too fresh -> not straight-line sell acceleration.
//   SHORT:
//     Up impulse -> reject -> high is not too fresh -> not straight-line buy acceleration.
//
// INSTALL
...
namespace NinjaTrader.NinjaScript.Strategies
{
    public class CG_T2_ClanMarshal_v9_2_LIGHT_B6_1_Decel : Strategy
    {
        ...
        [NinjaScriptProperty]
        [Range(0, 100)]
        [Display(Name = "PostExitTickLock", GroupName = "08 AntiRecursion", Order = 1)]
        public int PostExitTickLock { get; set; }

        [NinjaScriptProperty]
        [Range(0, 60000)]
        [Display(Name = "MinMillisecondsBetweenEntries", GroupName = "08 AntiRecursion", Order = 2)]
        public int MinMillisecondsBetweenEntries { get; set; }

        [NinjaScriptProperty]
        [Range(1, 500)]
        [Display(Name = "PendingEntryMaxBars", GroupName = "08 AntiRecursion", Order = 3)]
        public int PendingEntryMaxBars { get; set; }

        [NinjaScriptProperty]
        [Display(Name = "BlockSameBarReentry", GroupName = "08 AntiRecursion", Order = 4)]
        public bool BlockSameBarReentry { get; set; }

        [NinjaScriptProperty]
        [Range(1, 10)]
        [Display(Name = "CascadeEmergencyThreshold", GroupName = "09 Cascade", Order = 1)]
        public int CascadeEmergencyThreshold { get; set; }

        [NinjaScriptProperty]
        [Range(1, 120)]
        [Display(Name = "CascadeWindowMinutes", GroupName = "09 Cascade", Order = 2)]
        public int CascadeWindowMinutes { get; set; }

        [NinjaScriptProperty]
        [Range(1, 120)]
        [Display(Name = "CascadeSuppressMinutes", GroupName = "09 Cascade", Order = 3)]
        public int CascadeSuppressMinutes { get; set; }

        [NinjaScriptProperty]
        [Range(1, 100)]
        [Display(Name = "CascadeContinuationTicks", GroupName = "09 Cascade", Order = 4)]
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                Description = "MNQ LIGHT B6_1 Decel single series tick fade engine.";
            ...
            else if (State == State.DataLoaded)
            {
                ...
                if (PrintDiagnostics)
                    Print(Name + " loaded. Use on MNQ 1 tick chart. No AddDataSeries.");
            }
        }
        ...
        private void MaybePrintDiagnostics(double px, DateTime now)
        {
            ...
            Print(string.Format(
                "{0:yyyy-MM-dd HH:mm:ss} DIAG ticks={1} px={2:F2} pos={3} trades={4} submittedL={5} submittedS={6} Lsetup={7} Ssetup={8} LaccelBlk={9} SaccelBlk={10} sessBlk={11} coolBlk={12} govBlk={13} antiRecBlk={14} pendBlk={15} flipBlk={16}",
                now,
                ticksSeenToday,
                px,
        ...
        {
            Print("------------------------------------------------------------");
            Print(string.Format("{0:yyyy-MM-dd HH:mm:ss} B61 DAILY SUMMARY", now));
