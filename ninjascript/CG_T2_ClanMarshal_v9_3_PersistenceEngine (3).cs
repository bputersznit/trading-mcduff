...
// Generated: 2026-05-06 16:18:00 EDT
//
// PROJECT:
...
namespace NinjaTrader.NinjaScript.Strategies
{
    public class CG_T2_ClanMarshal_v9_3_PersistenceEngine : Strategy
    {
        ...
        protected override void OnBarUpdate()
        {
            ...
            // Momentum continuation entry for persistent expanding auctions.
            if (!submitted && auctionState == AuctionState.ConfirmedUp)
                submitted = TryMomentumLong(now, px);

            if (!submitted && auctionState == AuctionState.ConfirmedDown)
                submitted = TryTrendShort(now, px);

            // Momentum continuation short entry.
            if (!submitted && auctionState == AuctionState.ConfirmedDown)
        ...
        {
            // Persistent expanding auction:
            // allow participation even without ideal pullback.

            double breakoutTicks = (px - trendHighWater) / TickSize;

            if (auctionEnergy >= (EnergyConfirmThreshold + 4.0)
                && exhaustionScore < (ExhaustionThreshold * 1.50)
                && syntheticClose > priorSyntheticClose1
                && breakoutTicks >= -2)
            {
                SetStopLoss("B93_Trend_Long", CalculationMode.Ticks, TrendStopTicks, false);
                SetProfitTarget("B93_Trend_Long", CalculationMode.Ticks, TrendTargetTicks);
                ...
                if (PrintEntries)
                    Print(string.Format("{0:yyyy-MM-dd HH:mm:ss.fff} ENTER LONG Momentum px={1:F2} energy={2:F2}",
                        now, px, auctionEnergy));
        ...
        {
            double breakoutTicks = (trendLowWater - px) / TickSize;
                ...
                && breakoutTicks >= -2)
            {
                ...
                if (PrintEntries)
                    Print(string.Format("{0:yyyy-MM-dd HH:mm:ss.fff} ENTER SHORT Momentum px={1:F2} energy={2:F2}",
                        now, px, auctionEnergy));
