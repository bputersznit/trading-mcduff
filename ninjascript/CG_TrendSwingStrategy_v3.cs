...
namespace NinjaTrader.NinjaScript.Strategies
{
    ...
    public class CG_TrendSwingStrategy_v1 : Strategy
    {
            ...
            if (isSwingHigh)
            {
                ...
                if (swingRangeTicks >= MinSwingStrength && candidateHigh != lastConfirmedSwingHigh)
                {
                    ...
                    if (Position.MarketPosition == MarketPosition.Flat)
                    {
                        ...
                        pendingShortArmedPrimaryBar = BarsArray[0].GetBar(Times[1][0]);
            ...
            if (isSwingLow)
            {
                ...
                if (swingRangeTicks >= MinSwingStrength && candidateLow != lastConfirmedSwingLow)
                {
                    ...
                    if (Position.MarketPosition == MarketPosition.Flat)
                    {
                        ...
                        pendingLongArmedPrimaryBar = BarsArray[0].GetBar(Times[1][0]);
        ...
        private void CheckForTrendEntry()
        {
            ...
            {
                int barsSinceArm = pendingLongArmedPrimaryBar >= 0 ? CurrentBar - pendingLongArmedPrimaryBar : -1;
                ...
                if (pendingLongArmedTime != Core.Globals.MinDate && Time[0] >= pendingLongArmedTime
                    && barsSinceArm >= 0 && barsSinceArm <= EntryWindowBars && touchedPullbackZone && reclaimed)
                {
            ...
            {
                int barsSinceArm = pendingShortArmedPrimaryBar >= 0 ? CurrentBar - pendingShortArmedPrimaryBar : -1;
