...
namespace NinjaTrader.NinjaScript.Strategies
{
    public class CG_T2_ClanMarshal_LiveSignal_v1 : Strategy
    {
        ...
        [NinjaScriptProperty]
        [Display(Name = "StartTimeEt", Description = "HHmmss, e.g. 093100. Default avoids boundary/pre-open timestamp artifacts.", Order = 22, GroupName = "09. Session")]
        public int StartTimeEt { get; set; }

        [NinjaScriptProperty]
        [Range(0, 600)]
        [Display(Name = "RthStartBufferSeconds", Description = "Extra no-entry buffer after StartTimeEt to avoid boundary artifacts.", Order = 22, GroupName = "09. Session")]
        public int RthStartBufferSeconds { get; set; }
        ...
        private bool IsWithinRth()
        {
            ...
            if (now < StartTimeEt || now > EndTimeEt)
                return false;

            if (RthStartBufferSeconds <= 0)
                return true;

            DateTime start = BuildSessionDateTime(Time[0], StartTimeEt).AddSeconds(RthStartBufferSeconds);
            return Time[0] >= start;
        }

        private DateTime BuildSessionDateTime(DateTime baseTime, int hhmmss)
        {
            int hh = hhmmss / 10000;
            int mm = (hhmmss / 100) % 100;
            int ss = hhmmss % 100;
            return new DateTime(baseTime.Year, baseTime.Month, baseTime.Day, hh, mm, ss);
        }
