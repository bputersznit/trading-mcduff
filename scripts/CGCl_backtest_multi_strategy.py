...
def backtest_multi_strategy(date_str: str, signals: list[Signal]) -> list[Trade]:
    ...
    """
    print("\n" + "="*80)
    print(f"MULTI-STRATEGY BACKTEST: {date_str}")
    print("="*80)
    print(f"Position Size: 5 contracts")
    print(f"Cooldown: NONE (immediate re-entry)")
    print(f"Risk Limit: Rolling trades/hour < 4.0 (60-min window)")
    ...
    # Strategy-specific parameters
    PARAMS = {
        'ABSORPTION': {'target': 12.0, 'stop': 6.0, 'max_hold': 300},
        'ICEBERG': {'target': 10.0, 'stop': 5.0, 'max_hold': 180},
        'BREAKOUT': {'target': 15.0, 'stop': 7.0, 'max_hold': 120},
    }
...
def main():
    print("\n" + "╔" + "=" * 78 + "╗")
    print("║" + " " * 22 + "MULTI-STRATEGY BACKTEST" + " " * 33 + "║")
    print("╚" + "=" * 78 + "╝")
    ...
    print(f"\n{'Date':<12} {'Trades':<8} {'Win%':<8} {'Net P&L':<12}")
    print("-" * 80)

    for r in all_results:
        print(f"{r['date']:<12} {r['trades']:<8} {r['win_rate']:<7.1f}% ${r['net_pnl']:<11,.2f}")
