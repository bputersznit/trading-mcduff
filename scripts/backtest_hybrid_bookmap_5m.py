#!/usr/bin/env python3
"""
Hybrid Bookmap Strategy - 5M Timeframe
Combines Wall Rejection Reversal + Wall Break Momentum
ENFORCES: Only 1 MNQ position at a time
"""
...
class Position:
    """Track open position"""
    def __init__(self, strategy, direction, entry_price, entry_time, wall_price, entry_delta, entry_ratio=None):
...
class HybridBookmapStrategy:
    def __init__(self):
        ...
        self.position = None  # ONLY ONE POSITION ALLOWED
        self.trades = []
        self.active_walls = {}
        self.recent_prices = deque(maxlen=10)
        
        self.signal_stats = {
            'rejection_walls_detected': 0,
            'rejection_absorptions': 0,
            'rejection_entries': 0,
            'breakout_walls_detected': 0,
            'breakout_clearances': 0,
            'breakout_entries': 0,
            'position_blocked': 0  # Signals that couldn't enter due to existing position
        }
    ...
    def detect_rejection_signal(self, current_time, current_price, aggression):
        """Wall Rejection Reversal - for mean-reversion/range-bound"""
        if self.position:
                ...
                if current_price > wall.price - 3 and current_price < wall.price + 2:
                    if aggression['delta'] < -self.params['min_delta_for_absorption']:
                        if wall.price_low >= wall.price - 1.0:
                            ...
                            self.signal_stats['rejection_absorptions'] += 1
                            if current_price > wall.price and current_price > wall.price_low:
                                ...
                                self.signal_stats['rejection_entries'] += 1
    ...
    def detect_breakout_signal(self, current_time, current_price, walls, aggression):
        """Wall Break Momentum - for trend continuation"""
        if self.position:
            return None  # Position already open
        ...
        if ask_walls:
            self.signal_stats['breakout_clearances'] += 1
            # Check continuation aggression
            buy_ratio = aggression['buy_vol'] / max(aggression['sell_vol'], 1)
            if buy_ratio >= aggr_ratio and aggression['delta'] > self.params['min_delta_for_breakout']:
                wall_price = max(ask_walls, key=lambda x: x[1])[0]  # Use largest wall
                self.signal_stats['breakout_entries'] += 1
                return {
                    'strategy': 'breakout',
                    'direction': 'long',
        ...
        if bid_walls:
            ...
            if sell_ratio >= aggr_ratio and aggression['delta'] < -self.params['min_delta_for_breakout']:
    ...
    def check_exits(self, current_time, current_price, walls, aggression):
        ...
        # STOP LOSS (different buffers per strategy)
    ...
    def run_backtest(self, start_date, end_date):
        ...
        print(f"\n{'='*70}")
        print("HYBRID BOOKMAP STRATEGY - Wall Rejection + Wall Breakout")
        print(f"{'='*70}\n")
        ...
        for idx, row in df_aggr.iterrows():
            ...
            # Update wall tracking for rejection strategy
            for _, wall_row in current_walls.iterrows():
                ...
                if bid_liq >= self.params['wall_threshold']:
                    ...
                    if wall_key not in self.active_walls:
                        self.active_walls[wall_key] = WallState(price, bid_liq, 'bid', current_time)
                        self.signal_stats['rejection_walls_detected'] += 1
                
                if ask_liq >= self.params['wall_threshold']:
                    wall_key = (price, 'ask')
                    if wall_key not in self.active_walls:
                        self.active_walls[wall_key] = WallState(price, ask_liq, 'ask', current_time)
                        self.signal_stats['rejection_walls_detected'] += 1
            ...
            # Check entries (ONLY if no position exists)
            if not self.position:
                # Try rejection first (mean-reversion)
                rejection_signal = self.detect_rejection_signal(current_time, current_price, aggression)
                
                # Try breakout (trend-following)
                breakout_signal = self.detect_breakout_signal(current_time, current_price, current_walls, aggression)
                
                # Take whichever signal appears (rejection has priority if both)
                signal = rejection_signal or breakout_signal
    ...
    def print_results(self, daily_pnl, daily_trades):
        """Print results"""
        print("="*70)
        print("HYBRID BOOKMAP STRATEGY RESULTS")
        print("Wall Rejection Reversal + Wall Break Momentum")
        print("="*70)
        print()
        ...
        print("="*70)
        print()
        print("REJECTION (Mean-Reversion):")
        print(f"  Walls detected: {self.signal_stats['rejection_walls_detected']}")
        print(f"  Absorptions: {self.signal_stats['rejection_absorptions']}")
        print(f"  Entries: {self.signal_stats['rejection_entries']}")
        print()
        print("BREAKOUT (Trend-Following):")
        print(f"  Clearances detected: {self.signal_stats['breakout_clearances']}")
        print(f"  Entries: {self.signal_stats['breakout_entries']}")
        print()
        ...
        if self.trades:
            ...
            print("="*70)
            for strategy in ['rejection', 'breakout']:
                strat_trades = df_trades[df_trades['strategy'] == strategy]
                if len(strat_trades) > 0:
                    winners = strat_trades[strat_trades['gross_pnl'] > 0]
                    gross = strat_trades['gross_pnl'].sum()
                    net = strat_trades['net_pnl'].sum()
                    print(f"\n{strategy.upper()}:")
                    print(f"  Trades: {len(strat_trades)} | Winners: {len(winners)} ({len(winners)/len(strat_trades)*100:.1f}%)")
                    print(f"  Gross P&L: ${gross:+,.2f}")
                    print(f"  Net P&L: ${net:+,.2f}")
                    print(f"  Avg Gross: ${gross/len(strat_trades):+,.2f}")
                    print(f"  Avg Net: ${net/len(strat_trades):+,.2f}")
...
if __name__ == '__main__':
    ...
    strategy = HybridBookmapStrategy()
