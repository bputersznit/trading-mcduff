#!/usr/bin/env python3
"""
T2-Enhanced Strategy - 5S Timeframe
Key Features:
...
Tests on 5S bars to reduce noise and overtrading.
"""
...
class T2EnhancedBacktest:
    ...
    def get_market_snapshot(self, timestamp, current_price):
        ...
        # Use 5S heatmap table (uses contract symbol MNQZ5)
        wall_query = f"""
        ...
        aggr_query = f"""
            ...
            WHERE toDateTime(ts_bucket) >= toDateTime('{ts_str}') - INTERVAL 10 SECOND
              AND toDateTime(ts_bucket) <= toDateTime('{ts_str}')
        """
    ...
    def run_date(self, date_str):
        # Use 5S bars (uses contract symbol MNQZ5)
        bars_query = f"""
        ...
        for idx, bar in bars.iterrows():
            ...
            # Check for new signals (every bar on 5S)
    ...
    def generate_report(self):
        ...
        print("\n" + "="*70)
        print("T2-ENHANCED STRATEGY - 5S TIMEFRAME")
        print("="*70)
...
def main():
    ...
    params = {
        ...
        'max_bars_in_position': 60  # 60 bars @ 5S = 5 minutes max hold
    }
