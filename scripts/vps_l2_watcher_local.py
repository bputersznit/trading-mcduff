#!/usr/bin/env python3
"""
VPS L2 CSV Watcher - LOCAL ONLY VERSION
Monitors L2 capture folder for completed CSV chunks
Converts to Parquet, deletes CSV, keeps Parquet for Ubuntu to pull
"""
...
def convert_csv_to_parquet(csv_file: Path) -> Path:
    ...
    # Create parquet filename (same location as CSV)
...
def process_csv_chunk(csv_file: Path):
    """Convert CSV to Parquet and delete CSV (keep Parquet for Ubuntu to pull)"""
    ...
    try:
        ...
        # Delete CSV only (keep Parquet for Ubuntu)
        safe_delete(csv_file, "CSV")

        logger.info(f"  ✓ Ready for Ubuntu to pull: {parquet_file.name}\n")
        return True
...
def main():
    logger.info("=" * 60)
    logger.info("L2 CSV Watcher Started (LOCAL MODE - Ubuntu will pull)")
    logger.info(f"Monitoring: {L2_CAPTURE_PATH}")
    logger.info(f"Check interval: {CHECK_INTERVAL}s")
    logger.info("Parquet files stay on VPS for Ubuntu to pull")
    logger.info("=" * 60)
