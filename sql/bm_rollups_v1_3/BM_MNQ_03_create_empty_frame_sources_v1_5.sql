...
--
-- Why v1.5 exists:
--   v1.4 tried to build a unified keyset with UNION DISTINCT over very large
--   heatmap + aggression tables. That is logically correct but exceeded memory
--   at the 1S scale. v1.5 uses day-batched INSERTs instead.
--
-- Run:
--   clickhouse-client --multiquery < BM_MNQ_03_create_empty_frame_sources_v1_5.sql
...
SELECT
    ...
    (
        h.rth_flag = 1
        AND h.bid_liquidity_event_size >= d.daily_p999_heatmap_proxy_value
        AND h.bid_liquidity_event_size > h.ask_liquidity_event_size
        AND d.daily_p999_heatmap_proxy_value > 0,
        1,
        0
    ...
    (
        h.rth_flag = 1
        AND h.ask_liquidity_event_size >= d.daily_p999_heatmap_proxy_value
