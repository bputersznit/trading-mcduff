...
#
# Purpose:
#   Memory-safe frame-source repair for BM_MNQ_* Bookmap emulation.
#
# This script:
#   1. Recreates empty BM_MNQ_FRAME_SOURCE_1S/5S/30S/1M/5M tables.
#   2. Inserts heatmap-led rows one trade_date + scale at a time.
#   3. Inserts aggression-only rows one trade_date + scale at a time.
#   4. Runs frame QA.
#
# Why not UNION DISTINCT:
#   UNION DISTINCT over the full 1S heatmap/aggression keyset exceeded the
#   server memory limit. This script avoids the global keyset and works in
#   smaller day partitions.
#
# Usage:
...
echo "=== BM_MNQ frame repair batched v1_5 ==="
...
"$CH_CLIENT" --multiquery < BM_MNQ_03_create_empty_frame_sources_v1_5.sql
...
"$CH_CLIENT" --multiquery < BM_MNQ_05_frame_repair_qa_v1_5.sql

echo "[4/4] Done."
echo "=== BM_MNQ frame repair batched v1_5 complete ==="
