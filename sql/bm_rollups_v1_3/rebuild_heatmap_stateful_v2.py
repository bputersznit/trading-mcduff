#!/usr/bin/env python3
"""
rebuild_heatmap_stateful_v2.py
...
def rebuild_heatmap_for_date(client, trade_date):
    ...
    for bucket_idx, bucket_start in enumerate(time_buckets):
        ...
        if len(heatmap_rows) >= BATCH_SIZE:
            ...
            client.insert_df('BM_MNQ_HEATMAP_LIQUIDITY_EVENTS_100MS', heatmap_df)
...
def main():
    print("=== BM_MNQ Heatmap State-Based Rebuild v2 ===")
    ...
    client.command("DROP TABLE IF EXISTS BM_MNQ_HEATMAP_LIQUIDITY_EVENTS_100MS_EVENT_BASED_BACKUP")
    client.command("""
        CREATE TABLE BM_MNQ_HEATMAP_LIQUIDITY_EVENTS_100MS_EVENT_BASED_BACKUP
        ENGINE = MergeTree
